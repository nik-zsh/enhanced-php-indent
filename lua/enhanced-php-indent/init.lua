-- enhanced-php-indent.nvim - PHP-only indentation
local M = {}
...
-- Main PHP indent function (no HTML detection)
function _G.EnhancedPhpIndent()
    ...
    -- Control structures
    if
        prev_clean:find("^if%s*%(")
        or prev_clean:find("^while%s*%(")
        or prev_clean:find("^for%s*%(")
        or prev_clean:find("^foreach%s*%(")
        or prev_clean:find("^function%s")
        or prev_clean:find("^class%s")
        or prev_clean:find("^interface%s")
        or prev_clean:find("^trait%s")
    then
        return prev_indent + sw + base_indent
    end
